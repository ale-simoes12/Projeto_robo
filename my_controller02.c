...
posAnt = wb_supervisor_node_get_position(box);
  ...
  */
  ...
  pos = wb_supervisor_node_get_position(box);
  ...
  for(i=0;i<QtddSensoresProx;i++){
    ...
    //sprintf(texto,"At   %f %f %f  Ant   %f %f %f",pos[0], pos[1], pos[2],posAntX, posAntY, posAnt[2]);
    //printf("%s\n",texto);
    //sprintf(texto," %d",virando);
  ...
  if(pos[0] != posAnt[0] || pos[1] != posAnt[1]) {
    wb_led_set(Leds[0], 1);
    wb_led_set(Leds[1], 1);
    wb_led_set(Leds[2], 1);
    wb_led_set(Leds[3], 1);
  ...
  */
  ...
  if(sensor != -1){
    ...
    if(AceleradorDireito == 1 && AceleradorEsquerdo == 1){
      
      contV++;
      
    }
    
    if(contV % 6 == 0 || contV % 6 == 1 || contV % 6 == 2){
  ...
  if(LeituraSensorProx[sensor] <= 100){
    sensor = -1;
    virando = 0;
  }
